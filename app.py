...
def index():
    ...
    initial_stake = initial_stake_amount + (node_yield_per_48h * initial_nodes)
    node_yield_48h = node_yield_per_48h * initial_nodes

    # Create a DataFrame to store results
    columns = [
        "Month", "Month Abbr", "Node Count", "Staked (Neoxa)", "Just Node Yield (Neoxa)",
        "Total Yield (Neoxa)", "USD Value", "KD Value", "Running Total KD"
    ]
    data = []
    ...
    for month in range(total_months):
        ...
        total_yield = monthly_stake_yield + node_yield
        ...
        data.append([
            ...
            round(current_stake, neoxa_decimals), 
            round(node_yield, neoxa_decimals), 
            round(total_yield, neoxa_decimals), 
            round(usd_value, usd_decimals), 
            round(kd_value, kd_decimals), 
            round(total_kd, kd_decimals)
        ])
